"""QLoRA fine-tune Gemma 4 E2B to emit a fixed-length binned trajectory from a single image.

The answer is always WAYPOINTS*6 = 60 tokens: position w*6+j is joint j at waypoint w, and each
token is one of 256 bins. Nothing about the output can be malformed, unlike the variable-length
FAST/BPE encoding it replaces.
"""
...
def build_example(proc, img_path, bins, remap, n_base):
    ...
    enc["attention_mask"] = torch.cat([enc["attention_mask"], torch.ones_like(answer)], dim=1)
    enc["mm_token_type_ids"] = torch.cat([enc["mm_token_type_ids"], torch.zeros_like(answer)], dim=1)
    labels = enc["input_ids"].clone()
    labels[:, : prefix.shape[1]] = -100  # supervise the trajectory tokens only
    enc["labels"] = labels
    return enc
...
def main():
    ...
    ap.add_argument("--out", type=str, default=os.path.join(OUT, "adapter"))
    args = ap.parse_args()

    pairs = json.load(open(os.path.join(OUT, "pairs.json")))
    random.seed(0)
    random.shuffle(pairs)
    val, pairs = pairs[:40], pairs[40 : 40 + args.n_samples]
    json.dump(val, open(os.path.join(OUT, "val.json"), "w"))
    ...
    for ep in range(args.epochs):
        ...
        for i, p in enumerate(pairs):
            enc = build_example(proc, os.path.join(OUT, p["img"]), p["bins"], remap, n_base)
            out = model(**enc)
        ...
        print(f"[ep {ep}] checkpoint saved", flush=True)
