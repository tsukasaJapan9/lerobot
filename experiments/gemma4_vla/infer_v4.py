"""(image + current joint state) -> trajectory, v4 model, evaluated on held-out windows."""
...
def main():
    ...
    ap.add_argument("--adapter", default=os.path.join(OUT, "adapter_v4"))
    ap.add_argument("--n", type=int, default=6)
    ap.add_argument("--png", default=os.path.join(OUT, "traj_v4.png"))
    args = ap.parse_args()
    ...
    for row, p in enumerate(picks):
        ...
        state = torch.tensor([[n_base + b for b in p["state_bins"]]], device="cuda")
        enc["input_ids"] = torch.cat([remap[enc["input_ids"]], state], dim=1)
        enc["attention_mask"] = torch.cat([enc["attention_mask"], torch.ones_like(state)], dim=1)
        enc["mm_token_type_ids"] = torch.cat([enc["mm_token_type_ids"], torch.zeros_like(state)], dim=1)
        n_tok = len(p["bins"])
        with torch.no_grad():
